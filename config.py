...
# UTAGEで決済が起きるステップ。成約・売上の計算には使わず、「UTAGEでは決済があったのに
# 事務局リストに決済完了が無い」ことの検知だけに使う。統計の売上は分割払いの決済された回の分しか載らず、
# コース選択ページは3コースの決済フォームが同居していて区別できないため、契約額を出せない(2026-09-15)。
PAY_SALE_STEP_IDS = [
...
# 本講座の成約数・売上は【事務局管理用】講座お申込みリストで数える(2026-09-15 ユーザー決定)。
# 成約＝「決済完了」にチェックあり かつ「キャンセル」にチェックなし。成約日＝U列「決済完了日」。
# 売上＝コースの定価(分割払いでも定価で計上)。リストは今の状態しか持たないので、
# 過去日を再実行すると後から付いたキャンセルもその日に遡って反映される。
# ★B〜D列・T〜U列は手入力で行番号に紐づき、E〜S列はv2からQUERYで並ぶ。v2の行順が変わるとチェックが別人にずれる
COURSE_LIST_SHEET_NAME = "【事務局管理用】講座お申込みリスト"
COURSE_LIST_HEADER_ROW = 7
COURSE_LIST_COLUMNS = {  # 列 → 見出しの先頭文字列(見出しが違えば列ずれとみなして止める)
    "paid": ("B", "決済完了"),
    "name": ("E", "お名前"),
    "course": ("N", "お申込コース"),
    "cancel": ("T", "キャンセル"),
...
COURSE_PRICES = [("人生激変", 480000), ("覚醒大変身", 680000), ("10倍", 980000)]  # コース名に含まれる語 → 定価
