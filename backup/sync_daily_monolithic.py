#!/usr/bin/env python3
"""
★★★ このファイルは実行してはいけません（2026-09-15に実行を禁止しました）★★★

これは分割リファクタ(7ce9b68)の前の姿を残してあるだけの保管用コピーです。
2026-09-11の並べ替え(9691c97)より古い行番号(ROW)を持っており、しかも
「行11〜63を一括ブロック書き」する作りなので、走らせるとシートの列が丸ごと壊れます。

  ・セミナーLPのPV(2449)がウェビナーLPのPV行(11)に入る
  ・売上金額(240000)が視聴完了率の行(41)に入る
  ・行64〜73だけ無傷で残る（これが被害の指紋）

2026-09-12〜14の3日間、実際にこの形で3日分の列が壊れました。
しかも「列見出しの一致」「累計の逆行」チェックはどちらも行ズレを検知しないため、
事故に気づけたのは4日目に逆行チェックが偶然引っかかったからでした。

動かしたいのはリポジトリ直下の sync_daily.py です（config.py / sheets_client.py /
utage_api.py に分割されており、行ズレを検知する verify_row_alignment が入っています）。

--- 以下は当時のドキュメント（内容は古い可能性があります）---
...
# ★保管用コピーなので実行を禁止する（2026-09-15追加。理由は冒頭のコメント）。
#   行番号が2026-09-11の並べ替えより古く、走らせるとシートの列が丸ごと壊れる。
#   冒頭に注意書きを置くだけでは、読まずに実行された時に止められないため
#   import された時点で落とす。読むだけなら差し支えない。
if __name__ == "__main__":
    raise SystemExit(
        "このファイル(backup/sync_daily_monolithic.py)は実行できません。\n"
        "2026-09-11の並べ替えより古い行番号を持っているため、走らせるとシートの\n"
        "列が丸ごと壊れます（実際に2026-09-12〜14の3日分が壊れました）。\n"
        "リポジトリ直下の sync_daily.py を使ってください。"
    )
