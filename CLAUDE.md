...
* **データ取得処理と安全対策**:
  * 優先: `gspread`（サービスアカウント経由でリアルタイム取得、最大3回リトライ）
  * フォールバック: Google Sheets CSV export (`gviz/tq`、最大3回リトライ)
  * **空データ・通信エラー安全対策**: Google APIの503エラーや通信タイムアウト等でデータ取得に失敗した場合は、誤って「スケジュールなし」で空投稿するのを防ぐため、メッセージ送信を行わずにエラー終了（リトライ対象）とします。
